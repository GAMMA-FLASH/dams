...
void *acqThreadFcn(void *ptr) {
    ...
    // Make sure to have enough time to fill the buffer before the trigger
    ...
    for (;;) {
        ...
        // 6. Wait to fill the pre-trigger part of the waveform
        usleep(armDelayUsec);
        ...
        // 8. Wait for an event
        // Sometimes the trigger gets stuck, so the wait is repeated a fixed number of times
        ...
        if (res == 0) {
            ...
            TimeStamp ts;
            TimeStamp::CurrentTime currTime;
            uint32_t tsts = ts.read(&currTime);
            
            // Fill packet header
            wformBuff->currTime = currTime;
...
int WaveAcq::init() {
    ...
    if (res >= 0) {
        res = pthread_create(&acqThreadInfo, NULL, acqThreadFcn, this);
        if (res >= 0) {
            threadStarted = true;
